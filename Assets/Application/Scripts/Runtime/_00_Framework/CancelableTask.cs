...
{
	/// <summary>
	/// キャンセル可能なタスク Version 2022/06/04
	/// </summary>
	public class CancelableTask 
	{
		...
		// タスクキャンセル用トークン
		protected	CancellationToken			m_OwnerCancellationToken = default ;
		...
		{
			CancellationToken ownerCancellationToken = m_OwnerCancellationToken ;
		...
		public CancelableTask( ExMonoBehaviour owner )
		{
			if( owner != null )
			{
				...
				m_OwnerCancellationToken = owner.GetCancellationTokenOnDestroy() ;
				m_OwnerCancellationToken.Register( OnOwnerDestroyed ) ;
			}
		}
