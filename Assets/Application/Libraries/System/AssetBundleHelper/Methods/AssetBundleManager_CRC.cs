...
namespace AssetBundleHelper
{
	...
	public partial class AssetBundleManager
	{
		...
		// ＣＲＣ値を取得する(非同期)
		internal static IEnumerator GetCRC32Async( byte[] data, Action<uint> onResult, CancellationToken token = default )
		{
			// サブスレッドで計算を実行する
			Task<uint> task = Task.Run( () => GetCRC32_Task( data, token ) ) ; 
			if( task == null )
			{
				// 基本的にはここに来る事はないが念の為
				onResult?.Invoke( 0 ) ;
		...
		// ＣＲＣ値を取得する(サブスレッド)
		private static uint GetCRC32_Task( byte[] data, CancellationToken cancellationToken )
		{
			uint crc = CRC32_MASK  ;
			foreach( var code in data )
			{
